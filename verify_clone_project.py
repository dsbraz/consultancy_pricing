...
def verify_clone_project():
    ...
    if not profs:
        ...
        prof_data = {"pid": "TEST001", "name": "Test Prof", "role": "Dev", "level": "Senior", "hourly_cost": 100}
    ...
    # Add allocation
    alloc_res = requests.post(f"{BASE_URL}/projects/{project_id}/allocations/?professional_id={prof_id}")
    ...
    clone_details = requests.get(f"{BASE_URL}/projects/{cloned_id}/allocation_table").json()
    if len(clone_details["allocations"]) != 1:
        print(f"ERROR: Clone should have 1 allocation, found {len(clone_details['allocations'])}")
        sys.exit(1)
