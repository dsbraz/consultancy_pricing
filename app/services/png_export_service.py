...
class PNGExportService:
    ...
    def export_project_to_png(self, project: Project) -> BytesIO:
        ...
        y_position = self._draw_section_header(draw, "Tabela de Alocação", y_position, font_heading)
        y_position = self._draw_allocation_table(draw, project, y_position, font_small)
        y_position += 25
        
        y_position = self._draw_section_header(draw, "Resumo Financeiro", y_position, font_heading)
        y_position = self._draw_financial_summary(draw, project, y_position, font_normal)
    ...
    def _draw_title(self, draw, title, y_pos, font):
        """Draw project title"""
        draw.rectangle(
            [(0, y_pos - 10), (self.width, y_pos + 50)],
    ...
    def _draw_allocation_table(self, draw, project, y_pos, font):
        ...
        remaining_width = table_width - name_col_width - role_col_width - rate_col_width - total_col_width
        ...
        x_pos = start_x + 5
        draw.text((x_pos, header_y + 12), "Profissional", fill=self.text_color, font=font)
        x_pos += name_col_width
        
        draw.text((x_pos, header_y + 12), "Função/Nível", fill=self.text_color, font=font)
        x_pos += role_col_width
        
        draw.text((x_pos, header_y + 12), "Taxa (R$/h)", fill=self.text_color, font=font)
        x_pos += rate_col_width
        
        for week in weeks:
            week_label = f"S{week['week_number']}"
            draw.text((x_pos + 5, header_y + 12), week_label, fill=self.text_color, font=font)
            x_pos += cell_width
        ...
        for allocation in project.allocations:
            ...
            x_pos = start_x + 5
            draw.text((x_pos, y_pos + 8), professional.name[:24], fill=self.text_color, font=font)
            x_pos += name_col_width
            ...
            draw.text((x_pos, y_pos + 8), f"{allocation.selling_hourly_rate:.0f}", fill=self.text_color, font=font)
            x_pos += rate_col_width
            
            weekly_hours_map = {wa.week_number: wa.hours_allocated for wa in allocation.weekly_allocations}
            total_hours = 0
            ...
            for week in weeks:
                ...
                total_hours += hours
                if hours > 0:
                    draw.text((x_pos + 5, y_pos + 8), f"{hours:.0f}", fill=self.text_color, font=font)
                x_pos += cell_width
            
            draw.text((x_pos + 5, y_pos + 8), f"{total_hours:.0f}h", fill=self.text_color, font=font)
    ...
    def _draw_financial_summary(self, draw, project, y_pos, font):
        ...
        y_pos += 5
        draw.rectangle(
            [(self.padding + 10, y_pos - 5), (self.width - self.padding - 10, y_pos + 30)],
            fill=self.primary_color
        )
        
        draw.text((x_label, y_pos + 5), "PREÇO FINAL:", fill=(255, 255, 255), font=font)
        draw.text((x_value, y_pos + 5), f"R$ {pricing['final_price']:,.2f}", fill=(255, 255, 255), font=font)
