...
def verify_delete_professional():
    ...
    resp = requests.post(
        f"{BASE_URL}/projects/{project_id}/allocations/?professional_id={prof1_id}"
    )
    ...
    resp = requests.post(
        f"{BASE_URL}/projects/{project_id}/allocations/?professional_id={prof2_id}"
    )
    assert resp.status_code == 200
    ...
    data = resp.json()
    print(
        f"✓ After delete, allocation table has {len(data['allocations'])} professionals"
    )
