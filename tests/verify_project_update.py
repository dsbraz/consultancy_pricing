...
def verify_project_update():
    ...
    try:
        ...
        if updated_proj["name"] != update_data["name"]:
            errors.append(
                f"Name mismatch: expected {update_data['name']}, got {updated_proj['name']}"
            )
        if updated_proj["duration_months"] != update_data["duration_months"]:
            errors.append(
                f"Duration mismatch: expected {update_data['duration_months']}, got {updated_proj['duration_months']}"
            )
        if updated_proj["tax_rate"] != update_data["tax_rate"]:
            errors.append(
                f"Tax rate mismatch: expected {update_data['tax_rate']}, got {updated_proj['tax_rate']}"
            )
        if updated_proj["margin_rate"] != update_data["margin_rate"]:
            errors.append(
                f"Margin rate mismatch: expected {update_data['margin_rate']}, got {updated_proj['margin_rate']}"
            )
