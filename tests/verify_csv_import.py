...
VAC001,Test Template,Desenvolvedor,Júnior,true,80.00"""
    ...
    for prof in professionals:
        if prof["name"].startswith("Test"):
            print(
                f"  - [{prof.get('pid')}] {prof['name']}: {prof['role']} ({prof['level']}) - R$ {prof['hourly_cost']}"
            )
...
PROF002,Test Professional 2,Designer,Sênior,false,160.00"""
    ...
    try:
        ...
        ):  # IntegrityError usually returns 500 unless handled
            print(
                "✅ Uniqueness constraint verified (Server returned 500 as expected for IntegrityError)"
            )
        else:
