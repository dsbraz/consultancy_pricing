...
def verify_date_shift():
    ...
    if len(new_weeks) > 0:
        ...
        # Verify the shift
        initial_first_week_date = datetime.date.fromisoformat(
            initial_weeks[0]["week_start"]
        )
        new_first_week_date = datetime.date.fromisoformat(
            first_week_after["week_start"]
        )
        actual_shift = (new_first_week_date - initial_first_week_date).days

        print("\n   📊 ANÁLISE DO DESLOCAMENTO:")
        print("      Deslocamento esperado: 7 dias")
        print(f"      Deslocamento real: {actual_shift} dias")

        if actual_shift == 7:
            print("      ✅ Deslocamento correto!")
        else:
            print("      ❌ ERRO: Deslocamento incorreto!")
            sys.exit(1)
        ...
        for week_num in sorted(initial_weekly_hours.keys(), key=int):
            if week_num in new_weekly_hours:
                ...
                if initial_hours != new_hours:
                    print(
                        f"      ❌ Semana {week_num}: horas mudaram de {initial_hours} para {new_hours}"
                    )
                    hours_preserved = False

        if hours_preserved:
            print("      ✅ Todas as horas foram preservadas!")
        else:
            print("      ❌ ERRO: Algumas horas não foram preservadas!")
            sys.exit(1)

    # 9. Test duration change
    print("\n9. Testando mudança de duração (3 → 4 meses)...")
    update_data = {"duration_months": 4}
    make_request("PUT", f"/projects/{proj_id}", data=update_data)
    ...
    if len(weeks_after_duration) > len(new_weeks):
        print("      ✅ Novas semanas foram adicionadas!")
    else:
        print("      ❌ ERRO: Nenhuma semana foi adicionada!")
        sys.exit(1)
