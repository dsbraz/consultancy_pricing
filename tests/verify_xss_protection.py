...
def verify_xss_protection():
    ...
    resp = requests.post(f"{BASE_URL}/professionals/", json={
        "pid": "XSS001",
        "name": xss_payload,
        "role": "Developer",
        "level": "Senior",
    ...
    if data["name"] == xss_payload:
        print(f"✅ Script tag stored correctly (backend doesn't sanitize)")
    else:
    ...
    if found:
        print(f"✅ XSS payload retrievable from API")
    else:
        print(f"❌ XSS payload not found in professionals list")
        sys.exit(1)
    ...
    if data["role"] == xss_payload2:
        print(f"✅ IMG onerror stored correctly")
    else:
    ...
    if data["name"] == xss_payload3:
        print(f"✅ SVG onload stored correctly")
    else:
        print(f"❌ Unexpected name value: {data['name']}")
        sys.exit(1)
    ...
    resp = requests.post(f"{BASE_URL}/offers/", json={
        "name": xss_payload4,
        "items": []
    })
    ...
    if data["name"] == xss_payload4:
        print(f"✅ JavaScript protocol stored correctly")
    else:
        print(f"❌ Unexpected name value: {data['name']}")
        sys.exit(1)
