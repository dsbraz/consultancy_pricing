...
def verify_full_flow():
    ...
    prof1_resp = requests.post(f"{BASE_URL}/professionals/", json={
        "name": "Dev 1",
        "role": "Developer",
        "level": "Senior",
        "is_template": False,
    ...
    prof2_resp = requests.post(f"{BASE_URL}/professionals/", json={
        "name": "PM 1",
    ...
    offer_resp = requests.post(f"{BASE_URL}/offers/", json={
        "name": "Standard Squad",
        "items": [
            {"role": "Developer", "level": "Senior", "quantity": 2, "professional_id": prof1['id'], "allocation_percentage": 100.0},
            {"role": "Project Manager", "level": "Senior", "quantity": 1, "professional_id": prof2['id'], "allocation_percentage": 100.0}
        ]
    })
    ...
    apply_resp = requests.post(f"{BASE_URL}/projects/{project['id']}/apply_offer/{offer['id']}")
    ...
    try:
        price_resp = requests.get(f"{BASE_URL}/projects/{project['id']}/calculate_price")
